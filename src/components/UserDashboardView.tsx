...
export const UserDashboardView: React.FC<UserDashboardViewProps> = ({
    ...
    fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(rssUrl)}`)
      ...
      .then((data) => {
        if (data && data.status === 'ok' && Array.isArray(data.items)) {
          setFitnessNews(data.items.slice(0, 4));
        }
      })
      .catch(() => {})
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6 animate-fade-in pb-16">
      ...
      {/* 1. Top Executive Header / Stats Bar */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div 
          onClick={() => setCurrentTab('notes')}
          className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between cursor-pointer hover:border-amber-400 transition"
        >
          <div>
        ...
        <div 
          ...
          className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between cursor-pointer hover:border-indigo-400 transition"
        >
          <div>
            <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Tarefas Pendentes</div>
            <div className="text-xl font-black text-slate-900 dark:text-white mt-0.5">{pendingTasksCount} Pendente{pendingTasksCount !== 1 ? 's' : ''}</div>
          </div>
        ...
        <div 
          ...
          className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between cursor-pointer hover:border-orange-400 transition"
        >
          <div>
        ...
        <div 
          ...
          className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs flex items-center justify-between cursor-pointer hover:border-sky-400 transition"
        >
          <div>
      ...
      {/* 3. SEÇÃO PRINCIPAL DE PRODUTIVIDADE: Notas & Tarefas */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* A. Notas & Fixadas (2 Colunas) */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="text-[11px] text-slate-400">Acesso imediato às notas prioritárias</div>
              </div>
            </div>
            <button
              onClick={() => onOpenNewNote && onOpenNewNote()}
          ...
          <div className="space-y-3 flex-1">
            ...
            {pinnedNotes.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {pinnedNotes.map((note) => (
                  <div
                    ...
                    className="p-3.5 rounded-2xl bg-amber-50/70 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-900/40 hover:border-amber-400 transition cursor-pointer flex flex-col justify-between space-y-2 shadow-2xs group"
                  >
                    <div>
                      <div className="text-xs font-bold text-slate-900 dark:text-white truncate flex items-center gap-1.5">
                        <Pin className="w-3.5 h-3.5 fill-amber-500 text-amber-600 shrink-0" />
                        <span className="truncate">{note.title || 'Sem título'}</span>
                      </div>
                      <div className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-3 mt-1.5 leading-relaxed">
                        {note.content || 'Sem conteúdo adicional.'}
                      </div>
                    </div>

                    <div className="flex items-center justify-between pt-2 border-t border-amber-200/40 dark:border-amber-900/30 text-[10px] text-slate-400">
                      <span>{new Date(note.updated_at || note.created_at || Date.now()).toLocaleDateString('pt-BR')}</span>
                      {onTogglePin && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          className="p-1 rounded-lg hover:bg-amber-200/50 text-amber-600 transition"
              ...
              <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/40 text-center text-xs text-slate-400 space-y-1.5 border border-dashed border-slate-200 dark:border-slate-800">
                <p className="font-semibold text-slate-600 dark:text-slate-300">Nenhuma nota fixada no momento.</p>
                <p className="text-[11px]">Você pode fixar até 3 notas para mantê-las sempre visíveis no painel principal.</p>
              </div>
            )}
            ...
            {latestNote && !pinnedNotes.some(n => n.id === latestNote.id) && (
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Última Nota Modificada</div>
                <div
                  onClick={() => onEditNote && onEditNote(latestNote)}
                  className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer flex items-center justify-between gap-2"
                >
          ...
          >
            <span>Central de Notas KeepBoard</span>
            <span>Ver Todas as Notas ({notes?.length || 0}) →</span>
          </div>
        </div>
      ...
      {/* 4. SEÇÃO DE SAÚDE, JEJUM E TELEMETRIA DE ATIVIDADES */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* A. Gestão de Jejum Intermitente */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="text-[11px] text-slate-400">Motor de cetose e queima de gordura</div>
              </div>
            </div>
            <button
              onClick={() => setCurrentTab('fasting')}
            ...
            {activeFastingSession ? (
              <div className="space-y-3 p-3.5 bg-orange-50/60 dark:bg-orange-950/20 rounded-2xl border border-orange-200/60 dark:border-orange-900/30">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-black text-slate-900 dark:text-white font-mono bg-orange-500/10 px-2.5 py-1 rounded-lg text-orange-600">
                ...
                <div className="flex items-center gap-2 pt-1">
                  {onAddWater && (
                    <button
                      ...
                      className="flex-1 py-2 px-2.5 rounded-xl bg-sky-50 dark:bg-sky-950/40 text-sky-600 hover:bg-sky-100 text-xs font-semibold flex items-center justify-center gap-1 transition cursor-pointer border border-sky-200 dark:border-sky-900/40"
                    >
                  ...
                  {onEndFasting && (
                    <button
                      ...
                      className="py-2 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition cursor-pointer shadow-sm"
                    >
                      Concluir
                    </button>
                  )}
                  {onCancelFasting && (
                    <button
                      ...
                      className="py-2 px-3 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 hover:bg-rose-100 text-xs transition cursor-pointer border border-rose-200 dark:border-rose-900/40"
                      title="Cancelar Jejum"
                    >
                      ✕
                    </button>
                  )}
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 text-[11px] text-orange-600 dark:text-orange-400 font-semibold flex items-center justify-between">
            ...
            <span className="cursor-pointer hover:underline" onClick={() => setCurrentTab('fasting')}>Ver Histórico Completo →</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Previsão do Tempo 7 Dias (2 Colunas) */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4 lg:col-span-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5">
            {weatherLoading ? (
              Array.from({ length: 7 }).map((_, i) => (
                <div key={i} className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40 animate-pulse h-24" />
              ))
            ) : weather?.daily ? (
              weather.daily.time.slice(0, 7).map((dayStr, idx) => {
                const dateObj = new Date(dayStr + 'T00:00:00');
                const dayName = idx === 0 ? 'Hoje' : dateObj.toLocaleDateString('pt-BR', { weekday: 'short' });
                const max = Math.round(weather.daily?.temperature_2m_max[idx] || 0);
                const min = Math.round(weather.daily?.temperature_2m_min[idx] || 0);
                const rain = weather.daily?.precipitation_probability_max ? weather.daily.precipitation_probability_max[idx] : 0;
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded-2xl border flex flex-col items-center justify-between text-center transition ${
                      idx === 0
                        ? 'bg-sky-50/80 dark:bg-sky-950/30 border-sky-200 dark:border-sky-800 shadow-2xs'
                        : 'bg-slate-50/60 dark:bg-slate-800/40 border-slate-100 dark:border-slate-800'
                    }`}
                  >
                    <span className="text-[11px] font-bold uppercase text-slate-600 dark:text-slate-300">{dayName}</span>
                    <span className="text-lg my-1">{getWeatherText(code).slice(-2)}</span>
                    <div className="text-xs font-black text-slate-800 dark:text-slate-100">
                      {max}° <span className="text-[10px] text-slate-400 font-normal">{min}°</span>
                    </div>
                    {rain > 0 && (
                      <span className="text-[9px] font-semibold text-sky-500 mt-1 flex items-center gap-0.5">
              ...
              <div className="col-span-7 text-xs text-slate-400 text-center py-4">
                Informações de clima temporariamente indisponíveis.
              </div>
            )}
          </div>

          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[11px]">
            <span className="text-slate-400">Atualizado a cada 5 minutos</span>
            <button
              onClick={() => setCurrentTab('weather')}
              className="text-sky-600 dark:text-sky-400 font-semibold hover:underline cursor-pointer flex items-center gap-1"
            >
        ...
        {/* Notícias Fitness & Saúde (1 Coluna) */}
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Notícias Fitness</span>
                <div className="text-[11px] text-slate-400">Saúde e musculação</div>
              </div>
            </div>
            ...
            {newsLoading ? (
              ...
              fitnessNews.map((item, idx) => (
                <a
                  ...
                  className="flex items-start justify-between gap-2 p-2 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/80 hover:border-emerald-500/40 hover:bg-emerald-50/30 transition group text-xs"
                >
                  <div className="min-w-0 flex-1">
                    <h4 className="font-semibold text-slate-800 dark:text-slate-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition truncate leading-tight">
                      {item.title}
                    </h4>
                  </div>
                  <ExternalLink className="w-3.5 h-3.5 text-slate-400 group-hover:text-emerald-500 transition shrink-0" />
                </a>
              ))
            ) : (
      ...
      {/* 6. SEÇÃO INFERIOR: Calendário Nacional (BrasilAPI) */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md transition flex flex-col justify-between space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Calendário Mensal & Feriados Nacionais</span>
              <div className="text-[11px] text-slate-400">Integração oficial via BrasilAPI</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
            {holidays.filter(h => {
              ...
              holidays.filter(h => {
                ...
                <div key={idx} className="flex items-center justify-between text-xs p-2 rounded-xl bg-indigo-50/60 dark:bg-indigo-950/20 text-indigo-900 dark:text-indigo-200 border border-indigo-100 dark:border-indigo-900/30">
                  <span className="font-semibold truncate">{h.name}</span>
                  <span className="text-[10px] font-bold opacity-75 shrink-0 ml-2">{h.date.split('-').reverse().join('/')}</span>
                </div>
              ))
            ) : (
              <div className="text-xs text-slate-400 italic py-1 col-span-full">Nenhum feriado nacional registrado neste mês.</div>
            )}
          </div>
        </div>
      </div>
